...
这样做的原因是因为NestJS的依赖注入是有作用的，不同module中的资源如无特殊声明，是不允许被其他模块调用的。所以它要维护一个Map，每个module有自己的一套IoC容器。
...
这里面的代码更多，我们先专注于 `_providers` 这个私有属性，可以发现它的类型是： `Map<any, InstanceWrapper<Injectable>>` ，这就是Demo中的私有属性 `providers` ！当然，nestjs中的功能更加的完善，类型也更加的复杂。
